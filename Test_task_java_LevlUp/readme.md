...
### 6. Added BCrypt
	...
	С любой из страниц можно вернутся на главнию страницу.
		2. Запрос на регистрацию localhost:8080/registration открывается страница с формой для регистрации. Все поля за исключением фамилии
	врача являются обезательными для заполнения. На данный момент проверки существует только в форме регистрации.
	После заполнения всех обезательных полей отправляем форму. Данные будут формироватся из экземпляра PatientForm. В контроллере форма 
	...
	Что бы заполнить сущность генерируем рандомные логин и пароль они будут отправленны по указанной пациентом почте. Прежде чем добавить 
	...
	Идем далее, существует таблица предназначенная для врачей и другого адменистративного персонала. Создаем экземпляр. Просто берем данные 
