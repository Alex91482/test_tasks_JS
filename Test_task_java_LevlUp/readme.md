...
### 4. Added serialization and mailing lists to the project 
	...
	относится интерфейс MaintenanceSerialization который предназначен для определения дополнительных методов в классе PatientServiceImpl.
		...
		если хоть одно запись существует то ни каих добавлений записей не произойдет
	...
	сразу в бд (имеется ввиду что на основе записей из таблицы Patient будут созданы Recording для каждой такой записи).
